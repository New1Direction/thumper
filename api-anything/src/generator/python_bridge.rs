...
async fn discover_redmicro_root() -> Result<PathBuf> {
    ...
    let candidates = [
        ...
        "/opt/redmicro",
        "~/.grok/skills/redmicro",
    ];

    for c in &candidates {
        let expanded = shellexpand::tilde(c).into_owned();
    ...
    if harness_script.exists() {
        ...
        hcmd.arg(&harness_script)
            ...
            .arg(if req.description.is_empty() {
                "Absorbed tool via api-anything"
            } else {
                &req.description
            })
...
"""Basic absorption tests for {name} (generated by api-anything + absorb.py)."""
import json
import subprocess
import sys
...
"#,
    ...
    let readme_content = format!(
        "# Full Absorption Package for {}\n\n\
         Generated via api-anything generate --absorb\n\
